...
void MainWindow::primerNivel()
{
    ...
    scene->setBackgroundBrush(QImage(":/Backgrounds games/escenariocompleto.png"));
...
void MainWindow::segundoNivel()
{
    ...
    int iteradorGotitas = 0;
    while (iteradorGotitas < 8){
        gotita = new gotitas(0+(0*iteradorGotitas));
        listaGotitas.push_back(gotita);
        scene->addItem(gotita);
        iteradorGotitas += 1;
    }
    ...
    personaje_principal = new bolita(200,550,seleccion_personaje); //x,y,tamaño
    scene->addItem(personaje_principal);
    personaje_principal->setVidas(vidas);
...
void MainWindow::on_GuardarPartidaButton_3_clicked()
{
    if(guardar==0)
    {
        cout << "Guardar"<<endl;
...
void MainWindow::on_iniciarButton_2_clicked()
{
    segundoNivel();
}
